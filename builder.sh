...
get_package() {
    ...
    if $go_mod && [ -f /dnsproviders/$1/$1.go ]; then
        ...
        GO111MODULE=off GOOS=linux GOARM=7 GOARCH=arm caddyplug package $1 2> /dev/null
    fi
}
...
GOOS=linux GOARM=7 GOARCH=arm go get -v github.com/abiosoft/caddyplug/caddyplug
end_stage
